...
const Experience = () => {
    ...
    const renderExperienceCard = (item, index, isFreelance = false) => {
        return (
            <motion.div
                key={index}
                className={`mb-8 p-6 rounded-xl shadow-lg ${darkMode ? "bg-gray-800" : "bg-white"} border ${darkMode ? "border-gray-700" : "border-gray-200"}`}
                variants={cardVariants}
            >
                <div className="flex flex-col md:flex-row md:items-center mb-4">
                    <div>
                        <h3 className="text-2xl font-bold">{item.position}</h3>
                        <p className={`text-lg ${darkMode ? "text-blue-300" : "text-blue-600"}`}>{item.company}</p>
                    ...
                    <div className={`md:ml-auto mt-2 w-56 md:mt-0 px-4 py-2 rounded-full text-sm font-medium ${
                    ...
                    <ul className="space-y-2">
                        {item.responsibilities.map((responsibility, resIndex) => (
                            <li key={resIndex} className="flex items-start">
                                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 mr-2 mt-0.5 ${darkMode ? "text-blue-400" : "text-blue-500"}`} viewBox="0 0 20 20" fill="currentColor">
    ...
    return (
        <section id="experience-section" className={`relative py-16 ${darkMode ? "bg-gray-900" : "bg-white"} z-10`} style={{position: 'relative', zIndex: 1}}>
            {/* Debug border */}
            <div className=" py-8 container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    <h2 className={`text-3xl font-bold mb-4 ${darkMode ? "text-blue-400" : "text-blue-600"}`}>Professional Experience</h2>
